...
def getAllDetachments(clientID):
  #sql = "SELECT * FROM FieldEmployees"
  #res = getAll(sql)
  res = List("Detachments")
  FieldEmployeeList = []
  for row in res:
    if row is not None:
      #FieldEmployeeList.append([ str(row[0]), str(row[1]), str(row[2]), str(row[3]), str(row[4]), str(row[5]), str(row[6]), str(row[7]), str(row[8]), str(row[9]), str(row[10]), str(row[11]), str(row[12]), str(row[13]), str(row[14]), str(row[15]), str(row[16]), str(row[17]), str(row[18]), str(row[19]), str(row[20]) ])
...
def getOneFieldEmployee():
  sql = "SELECT * FROM FieldEmployees WHERE ID = %s"
  params = ("1")
  res = getOne(sql, params)
  res = str(res)
  FieldEmployees = []
  ctr = 0
  for row in res:
    FieldEmployees[ctr] = [ str(row[0]), str(row[1]), str(row[2]), str(row[3]), str(row[4]), str(row[5]), str(row[6]), str(row[7]), str(row[8]), str(row[9]), str(row[10]), str(row[11]), str(row[12]), str(row[13]), str(row[14]), str(row[15]), str(row[16]), str(row[17]), str(row[18]), str(row[19]), str(row[20]) ]
    ctr = ctr + 1
  print FieldEmployees[0]
